...
void handleRoot() {
  ...
  html += "document.addEventListener('keydown', function(event) { "
          ...
          "if (!pressed && event.key === 'ArrowRight') { pressed=true;fetch('/spinWeapon'); } "
          "if (!pressed && event.key === 'ArrowLeft') { pressed=true;fetch('/stopWeapon'); } });";
...
void setup() {
  ...
  server.on("/spinWeapon", HTTP_GET, spinWeaponHandler);
  server.on("/stopWeapon", HTTP_GET, stopWeaponHandler);
...
void spinWeaponHandler() {
  digitalWrite(GPIO5, HIGH);
  digitalWrite(GPIO6, LOW);
}

void stopWeaponHandler() {
  digitalWrite(GPIO5, LOW);
  digitalWrite(GPIO6, LOW);
}
